          ...
          <div className="space-y-3 text-xs">
            <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 space-y-2">
              ...
              <ul className="space-y-1.5 text-muted-foreground list-disc pl-4">
                <li>
                  <span className="font-semibold text-foreground">Top Category Driver:</span> Apparel & Footwear sales grew by <span className="text-emerald-400 font-semibold">18%</span> due to seasonal demand.
                </li>
                <li>
                  <span className="font-semibold text-foreground">Average Order Value (AOV):</span> Shifted to {currencySymbol}1,450 (up <span className="text-emerald-400 font-semibold">{currencySymbol}180</span>) due to accessory cross-selling.
                </li>
                <li>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="p-3.5 rounded-2xl bg-secondary/40 border border-border/40 space-y-1">
                ...
                <p className="text-xs text-emerald-400 font-semibold">{currencySymbol}{mostProfitable?.revenue.toLocaleString('en-IN') || 0} Total Volume</p>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-secondary/40 border border-border/40 space-y-1">
                ...
                <p className="text-xs text-emerald-400 font-semibold">{highMarginLowSales[0]?.margin.toFixed(0) || 55}% Margin / Low Volume</p>
              </div>
            </div>
          </div>
        ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/25 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-semibold text-emerald-400 uppercase tracking-wider">Most Profitable Product</span>
                ...
                <p className="text-muted-foreground">
                  Net Profit: <span className="font-bold text-emerald-400">{currencySymbol}{Math.round(mostProfitable?.profit || 0).toLocaleString('en-IN')}</span> ({mostProfitable?.margin.toFixed(0) || 45}% margin)
                </p>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-rose-500/10 border border-rose-500/25 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-semibold text-rose-400 uppercase tracking-wider">Lowest Margin Risk</span>
                  <Badge variant="outline" className="text-rose-400 border-rose-500/30 text-[10px]">Low Margin</Badge>
                </div>
                <h5 className="font-bold text-foreground">{leastProfitable?.product.name || 'Basic USB-C Cable'}</h5>
                <p className="text-muted-foreground">
                  Margin: <span className="font-bold text-rose-400">{leastProfitable?.margin.toFixed(0) || 12}%</span> — Consider price increase or supplier negotiation.
                </p>
              </div>
            </div>
            ...
            {highRevLowMargin.length > 0 && (
              <div className="p-3.5 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="font-semibold text-rose-400 block">High Volume / Low Margin Risk</span>
                ...
                <Button size="sm" variant="outline" className="rounded-xl text-xs shrink-0 border-rose-500/30 text-rose-400 hover:bg-rose-500/10">
                  Adjust Price
                </Button>
              </div>
            )}
